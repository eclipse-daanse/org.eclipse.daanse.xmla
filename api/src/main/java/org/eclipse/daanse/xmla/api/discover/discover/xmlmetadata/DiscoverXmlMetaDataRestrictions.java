...
public interface DiscoverXmlMetaDataRestrictions {
     ...
     */
    // @Restriction(name = RESTRICTIONS_MINING_MODEL_PERMISSION_ID, type =
    // XSD_STRING)
     ...
     */
    // @Restriction(name = RESTRICTIONS_MINING_STRUCTURE_PERMISSION_ID, type =
    // XSD_STRING)
     ...
     */
    // @Restriction(name = RESTRICTIONS_DATA_SOURCE_PERMISSION_ID, type =
    // XSD_STRING)
    Optional<String> dataSourcePermissionId();

    /**
     * @return The degree of expansion that is wanted in the return result. The available values are:
     *         ReferenceOnly - Returns only the name/ID/timestamp/state requested for the requested
     *         objects and all descendant major objects recursively. ObjectProperties - Expands the
     *         requested object with no references to contained objects (includes expanded minor
     *         contained objects). ExpandObject - Same as ObjectProperties, but also returns the name,
     *         ID, and timestamp for contained major objects. ExpandFull - Fully expands the requested
     *         object recursively to the bottom of every contained object.
     */
