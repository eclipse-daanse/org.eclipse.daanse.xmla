...
class MsXmlAnalysisSoapTest {
    ...
    void testDiscoverRequest() throws Exception {
        ...
        assertThat(discover).isNotNull().satisfies(d -> {
            // getRequestType
            assertThat(d.getRequestType()).isNotNull().isEqualTo("TEST_VALUE");
            // getRestrictions
            assertThat(d.getRestrictions()).isNotNull().satisfies(r -> {
                assertThat(r.getRestrictionMap()).isNotNull().hasSize(3).containsEntry("CATALOG_NAME", "catalogName")
                        .containsEntry("CUBE_NAME", "cubeName")
                        .containsEntry("DIMENSION_UNIQUE_NAME", "dimensionUniqueName");
            });
            // getProperties
            assertThat(d.getProperties()).isNotNull().satisfies(r -> {
                assertThat(r.getPropertyList()).isNotNull().satisfies(pl -> {
    ...
    void testResponse() throws Exception {
        ...
        doReturn(discoverResponse).when(xmlaWs).discover(Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any());
        ...
        xmlAssert.nodesByXPath("/SOAP:Envelope/SOAP:Body/msxmla:DiscoverResponse/msxmla:return/rowset:root").exist();
